#ifndef _NETCOMGRP_CENTRALIZED_PROTOCOL_HEADER_H_
#define _NETCOMGRP_CENTRALIZED_PROTOCOL_HEADER_H_
...
namespace protocol {
	...
	public:
		...
		// Address of the sender or receiver, depending on the context
		inline const address_tuple &addr() const;
		inline void                 addr(const address_tuple &a);
	...
	inline header::header() 
	    // magic_id is something to recognize netcomgrp centralized
	    // packets ('nc')
	  : _magic_id(0x6E63),	
	  	// 0.1
	    _version(0 << 8 | 1 << 0),
	...
	inline ACE_UINT16 header::magic_id() const { return ntohl(_magic_id); }
	
	// inline ACE_UINT32 header::time()    const { return ntohl(_time); }

	inline const address_tuple &header::addr() const { return _addr; }
	inline void  header::addr(const address_tuple &a) { _addr = a; }
...
#endif //_NETCOMGRP_CENTRALIZED_PROTOCOL_HEADER_H_
